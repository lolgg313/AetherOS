...
| 🎨 **[`OS.html`](OS.html)** | Desktop UI | **AetherOS 10 Lumina**, the whole desktop in one offline file: window manager, taskbar and Start menu, draggable desktop icons, and a virtual file system with a Recycle Bin and autosave. Apps: C Studio, Command Prompt, Notepad, Paint, Calculator, Explorer, Settings (with an encrypted vault), SoundWave, Cinema, Gallery with a 3D viewer, PDF and Word readers, the Aether Web browser, My Computer, Task Manager, App Studio (Lua apps), PyStudio (an IDLE-style Python 3 IDE: classes, generators, exceptions, comprehensions, f-strings and the standard modules, plus a DOM tkinter with pack/grid/place, Canvas, Menus and ttk; `.py` files open in it and programs can be saved as desktop apps) and the Supe3D FPS Creator. It bundles Fengari (Lua) and Three.js, so it needs no internet. It must sit next to `OS.py`. |
